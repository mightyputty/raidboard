...
class LogWatcher extends EventEmitter {
  ...
  readNotifications(file) {
    ...
    while ((m = NOTIFICATION_BLOCK.exec(txt))) {
      ...
      if (msg.type !== MSG_QUEST_STARTED && msg.type !== MSG_QUEST_FAILED &&
          msg.type !== MSG_QUEST_FINISHED) continue;
      ...
      // the text field reads "quest started" even on a failure; only the type is honest
      const status = msg.type === MSG_QUEST_FINISHED ? "d"
        : msg.type === MSG_QUEST_FAILED ? "f" : "a";
  ...
  snapshot() {
    ...
    for (const v of this.quests.values()) {
      if (v.status === "d") done++;
      else if (v.status === "f") failed++;
      else active++;
    }
    return {
      status,
      raid: this.raid,
      counts: { active, done, failed, tracked: this.quests.size },
      range: this.range,
      sessions: this.sessions,
      root: this.root,
      // The folder can appear after launch — someone installs the game, or points us somewhere that
      // does not exist yet — so this is checked per snapshot rather than once at startup.
      rootMissing: !fs.existsSync(this.root)
    };
  }
}
